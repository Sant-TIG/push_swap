...
static void	ft_3s_b(t_stack **stack_a, \
		t_stack **stack_b, t_pushswap **utils, int len, t_stacks *stacks)
{
	while (len--)
	{
		if ((*stack_b)->nbr < (*stack_b)->next->nbr)
			...
			ft_swap_a(stack_a);
		ft_push_a(stack_a, stack_b, utils, stacks);
	}	
}

static void	ft_3_b(t_stack **st_a, \
		t_stack **st_b, t_pushswap **utils, t_stacks *stacks)
{
	t_stack	*last;
	...
	ft_push_a(st_a, st_b, utils, stacks);
	ft_push_a(st_a, st_b, utils, stacks);
	ft_push_a(st_a, st_b, utils, stacks);
}
...
static void	ft_general_cases_b(t_stacks *stacks, t_stack *copy, int len, int *i)
{
	...
	while (max_push)
	{
		...
		{
			tmp = tmp->next;
			ft_push_a(&stacks->stack_a, &stacks->stack_b, &stacks->utils, stacks);
			max_push--;
		}
		else
		{
...
int	ft_check_sorted_b3(t_stack *stack_b)
{
	t_stack	*last;
...
{	
	...
	else if (len <= 3)
	{
		if (len == 2)
		{
			...
			ft_3_b(&stacks->stack_a, &stacks->stack_b, &stacks->utils, stacks);
		else if (len == 3)
			ft_3s_b(&stacks->stack_a, &stacks->stack_b, &stacks->utils, len, stacks);
		return (1);
	}
	return (0);
}
...
void	ft_sort_b(t_stacks *stacks, int len)
{
	...
	len1 = len / 2;
	i = 0;
	ft_copy_list_push(&copy, stacks->stack_b, len, stacks);
