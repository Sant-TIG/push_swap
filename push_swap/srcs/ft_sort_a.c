...
static int	ft_ut(t_stack **st_a, t_stack **st_b, t_pushswap **utils, int len, t_stacks *stacks)
{
	t_stack	*last;
	...
	{
		ft_push_b(st_a, st_b, utils, stacks);
		ft_swap_a(st_a);
...
static void	ft_treat_three_special_case_a(t_stack **st_a, \
		t_stack **st_b, t_pushswap **utils, int len, t_stacks *stacks)
{
	t_stack	*last;

	last = *st_a;
	if (ft_ut(st_a, st_b, utils, len, stacks))
		return ;
	while (--len)
		last = last->next;
	if ((*st_a)->nbr > (*st_a)->next->nbr && (*st_a)->nbr > last->nbr
...
static void	ft_general_cases_a(t_stacks *stacks, t_stack *copy, int len, int *i)
{
	...
	while (max_push)
	{
		...
		{
			tmp = tmp->next;
			ft_push_b(&stacks->stack_a, &stacks->stack_b, &stacks->utils, stacks);
			max_push--;
		}
		else
		{
...
void	ft_sort_a(t_stacks *stacks, int len)
{
	...
	i = 0;
	len1 = len / 2;
	ft_copy_list_push(&copy, stacks->stack_a, len, stacks);
